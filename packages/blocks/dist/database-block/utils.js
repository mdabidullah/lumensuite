    ...
    kanban(columnMetaMap, model, id, name) {
        ...
        const getWeight = (column) => {
            if ([
                ...
                columnModelPresets.selectColumnModelConfig.type,
            ].includes(column.type)) {
                return 3;
            }
            if ([
                columnModelPresets.numberColumnModelConfig.type,
...
export const databaseViewInitEmpty = (model, viewMeta) => {
    addColumn(model, 'start', titlePureColumnConfig.create(titlePureColumnConfig.config.name));
    databaseViewAddView(model, viewMeta);
};
export const databaseViewInitConvert = (model, viewMeta) => {
    addColumn(model, 'end', columnPresets.multiSelectColumnConfig.create('Tag', { options: [] }));
    databaseViewInitEmpty(model, viewMeta);
};
export const databaseViewInitTemplate = (model, viewMeta) => {
    const ids = [nanoid(), nanoid(), nanoid()];
    const statusId = addColumn(model, 'end', columnPresets.selectColumnConfig.create('Status', {
        options: [
            {
                id: ids[0],
                color: getTagColor(),
...
export function addColumn(model, position, column) {
...
export function applyColumnUpdate(model) {
    model.doc.updateBlock(model, {
...
export function copyCellsByColumn(model, fromId, toId) {
    model.doc.transact(() => {
...
export function findColumnIndex(model, id) {
...
export function getColumn(model, id) {
...
export function updateColumn(model, id, updater) {
