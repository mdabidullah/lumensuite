...
class PasteTr {
    constructor(std, text, snapshot) {
        ...
        this.focusPasted = () => {
            ...
            const cursorBlock = this.fromPointState.model.flavour === 'affine:code' || !this.lastSnapshot
        ...
        this.pasted = () => {
            ...
            if (this.to) {
                ...
                for (const model of context.selectedModels ?? []) {
                    if ([this.endPointState.model.id, this.fromPointState.model.id].includes(model.id) ||
            ...
            this.std.doc.moveBlocks(this.std.doc
                ...
                .slice(0, this.pasteStartModelChildrenCount), this.fromPointState.model);
            if (!this.firstSnapshotIsPlainText &&
                this.fromPointState.text.length == 0) {
    ...
    async _transformLinkDelta(delta, linkToDocId, quickSearchService) {
        ...
        const newDelta = delta.map(op => {
            if (needToConvert.has(op)) {
                ...
                const isLinkToNode = Boolean(extracted?.params?.mode &&
