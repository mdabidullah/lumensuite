...
import { getSortedCloneElements, serializeElement, } from '../utils/clone-utils.js';
...
export class EdgelessClipboardController extends PageClipboard {
    ...
    constructor(host) {
        ...
        this._onPaste = async (_context) => {
            ...
            if (isUrlInClipboard(data)) {
                ...
                if (pageId) {
                    ...
                    isLinkToNode = Boolean(extracted?.params?.mode &&
        ...
        this.registerBlock('affine:embed-linked-doc', this._createLinkedDocEmbedBlock);
        this.registerBlock('affine:embed-synced-doc', this._createSyncedDocEmbedBlock);
    ...
    async _createAttachmentBlock(attachment) {
    ...
    _createCanvasElement(clipboardData, oldToNewIdsMap, newXYWH) {
        if (clipboardData.type === GROUP) {
            ...
            for (const [key, value] of Object.entries(children)) {
                ...
                assertExists(newKey, 'Copy failed: cannot find the copied child in group');
            ...
            for (const [oldKey, oldValue] of Object.entries(children)) {
                ...
                assertExists(newKey, 'Copy failed: cannot find the copied node in mind map');
                if (oldValue.parent) {
                    const newParent = oldToNewIdsMap.get(oldValue.parent);
                    assertExists(newParent, 'Copy failed: cannot find the copied node in mind map');
    ...
    async _createEdgelessTextBlock(edgelessText) {
        ...
        const newId = await this.onBlockSnapshotPaste(edgelessText, this.doc, this.edgeless.surface.model.id);
        if (!newId) {
    ...
    _createFigmaEmbedBlock(figmaEmbed) {
    ...
    _createFrameBlock(frame, oldToNewIdMap) {
    ...
    _createGithubEmbedBlock(githubEmbed) {
    ...
    _createHtmlEmbedBlock(htmlEmbed) {
    ...
    async _createImageBlock(image) {
        const { xywh, rotate, sourceId, size, width, height, caption } = image.props;
        if (!(await this.host.std.collection.blobSync.get(sourceId))) {
            return null;
        }
        return this.host.service.addBlock('affine:image', {
            caption,
            sourceId,
            xywh,
            rotate,
    ...
    _createLinkedDocEmbedBlock(linkedDocEmbed) {
    ...
    _createLoomEmbedBlock(loomEmbed) {
    ...
    async _createNoteBlock(note) {
        ...
        const newId = await this.onBlockSnapshotPaste(note, this.doc, this.doc.root.id);
        if (!newId) {
    ...
    _createSyncedDocEmbedBlock(syncedDocEmbed) {
    ...
    _createYoutubeEmbedBlock(youtubeEmbed) {
    ...
    async _pasteTextContentAsNote(content) {
        ...
        if (typeof content === 'string') {
            ...
            for (let index = 0; index < content.length; index++) {
                await this.onBlockSnapshotPaste(content[index], this.doc, noteId, index);
            }
        }
    ...
    _replaceRichTextWithSvgElement(element) {
    ...
    _updatePastedElementsIndex(elements, originalIndexes) {
        ...
        sortedElements.forEach(ele => {
            const newIndex = idxGenerator(isTopLevelBlock(ele) ? ele.flavour : ele.type);
    ...
    async createElementsFromClipboardData(elementsRawData, pasteCenter) {
        ...
        for (const data of elementsRawData) {
            ...
            if (blockSnapshot) {
                ...
                const config = this._blockConfigs.find(({ flavour }) => flavour === blockSnapshot.flavour);
                if (!config)
                    continue;
                ...
                const newId = await config.createFunction(blockSnapshot, oldIdToNewIdMap);
                if (!newId)
                    continue;
                ...
                assertType(data);
                const oldId = data.id;
                const element = this._createCanvasElement(data, oldIdToNewIdMap, getNewXYWH(data.xywh));
    ...
    registerBlock(flavour, createFunction) {
