...
import { SURFACE_IMAGE_CARD_HEIGHT, SURFACE_IMAGE_CARD_WIDTH, } from '../../../image-block/components/image-block-fallback.js';
...
export function calcAngle(target, point, offset = 0) {
    ...
    return CommonUtils.normalizeDegAngle(((Vec.angle(center, point) + offset) * 180) / Math.PI);
}
export function calcAngleWithRotation(target, point, rect, rotate) {
    ...
    return CommonUtils.normalizeDegAngle((Vec.angle([c.x, c.y], point) * 180) / Math.PI);
}
