...
export class ShapeToolController extends EdgelessToolController {
    ...
    _addNewShape(e, width, height) {
        ...
        const attributes = this._edgeless.service.editPropsStore.lastProps$.value[`shape:${shapeName}`];
    ...
    createOverlay() {
        ...
        const { shapeName } = this.tool;
        const attributes = this._edgeless.service.editPropsStore.lastProps$.value[`shape:${shapeName}`];
        options.stroke = ThemeObserver.getColorValue(attributes.strokeColor, DEFAULT_SHAPE_STROKE_COLOR, true);
        options.fill = ThemeObserver.getColorValue(attributes.fillColor, DEFAULT_SHAPE_FILL_COLOR, true);
