...
export class DefaultToolController extends EdgelessToolController {
        ...
        if (this.edgelessSelectionManager.isInSelectedRect(x, y)) {
            if (this.edgelessSelectionManager.selectedElements.length === 1) {
                ...
                if (isCanvasElement(selected) &&
                    ...
                    selected.labelIncludesPoint(this._service.viewport.toModelCoord(x, y))) {
    ...
    _moveContent([dx, dy], alignBound, shifted, shouldClone) {
        ...
        if (this._draggingSingleMindmap) {
            ...
            if (hoveredMindmap) {
                ...
                element.opacity = 0.8;
                const { clear, mergeInfo } = MindmapUtils.showMergeIndicator(mindmap, node, subtree, [x, y]) ?? {};
                clear && this._clearMindMapHoverState.push(clear);
                const clearHide = MindmapUtils.hideTargetConnector(currentMindmap, subtree);
        ...
        this._hoveredFrame = this._edgeless.service.frame.getFrameFromPoint(this._service.viewport.toModelCoord(this._dragLastPos[0], this._dragLastPos[1]), this._toBeMoved.filter(ele => isFrameBlock(ele)));
        this._hoveredFrame
    ...
    initializeDragState(e, dragType) {
        ...
        if (this.dragType === DefaultModeDragType.Selecting) {
            this._disposables.add(this._service.viewport.viewportUpdated.on(() => {
                if (this.dragType === DefaultModeDragType.Selecting &&
                    this._dragging &&
                    !this._autoPanTimer) {
                    this._updateSelectingState();
        ...
        if (this.dragType === DefaultModeDragType.ContentMoving) {
            this._disposables.add(this._service.viewport.viewportMoved.on(delta => {
                if (this.dragType === DefaultModeDragType.ContentMoving &&
                    this._dragging &&
                    !this._autoPanTimer) {
    ...
    onContainerDragEnd() {
        ...
        if (this._draggingSingleMindmap) {
            if (this._hoveredMindMap && this._hoveredMindMap.mergeInfo) {
                ...
                MindmapUtils.moveMindMapSubtree(currentMindmap, currentNode, mindmap, mergeInfo.target, mergeInfo.index, mergeInfo.layoutType);
            }
        ...
        }
        {
            ...
            if (this._hoveredFrame) {
                ...
                toBeMovedTopElements.map(element => frameManager.removeParentFrame(element));
            }
        }
        if (this._lock) {
            this._doc.captureSync();
    ...
    async onContainerDragStart(e) {
        ...
        elements.forEach(element => {
            if (element.group instanceof MindmapElementModel && elements.length > 1) {
                getAllDescendantElements(element.group).forEach(ele => toBeMoved.add(ele));
            }
            else {
                getAllDescendantElements(element).forEach(ele => {
                    if (ele.group instanceof MindmapElementModel) {
                        getAllDescendantElements(ele.group).forEach(_el => toBeMoved.add(_el));
                    }
                    toBeMoved.add(ele);
                });
            }
    ...
    onContainerMouseMove(e) {
        ...
        if (isFrameBlock(hovered) &&
            hovered.externalBound?.isPointInBound(this._service.viewport.toModelCoord(e.x, e.y))) {
