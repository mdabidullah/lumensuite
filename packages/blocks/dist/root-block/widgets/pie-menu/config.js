...
shapes.forEach(shape => {
    pie.command({
        label: shape.label,
        icon: ({ rootComponent }) => {
            const attributes = rootComponent.service.editPropsStore.lastProps$.value[`shape:${shape.type}`];
...
pie.command({
    label: 'Toggle Style',
    icon: ({ rootComponent }) => {
        const { shapeStyle } = rootComponent.service.editPropsStore.lastProps$.value['shape:roundedRect'];
