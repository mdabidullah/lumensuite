...
import { createConversionItem, createDatabaseBlockInNextLine, formatDate, formatTime, insertContent, insideDatabase, insideEdgelessText, tryRemoveEmptyLine, } from './utils.js';
    ...
    items: [
        ...
        })),
        {
            ...
            action: ({ rootComponent }) => {
                ...
                if (!textSelection || !textSelection.isCollapsed())
                    return;
                const blockComponent = rootComponent.std.view.getBlock(textSelection.from.blockId);
                if (!blockComponent)
                    return;
        ...
        },
        {
            ...
            showWhen: ({ rootComponent, model }) => {
                const linkedDocWidgetEle = rootComponent.widgetComponents['affine-linked-doc-widget'];
                if (!linkedDocWidgetEle)
                    return false;
                const hasLinkedDocSchema = model.doc.schema.flavourSchemaMap.has('affine:embed-linked-doc');
                if (!hasLinkedDocSchema)
                    return false;
                if (!('showLinkedDocPopover' in linkedDocWidgetEle)) {
                    console.warn('You may not have correctly implemented the linkedDoc widget! "showLinkedDoc(model)" method not found on widget');
                    return false;
                }
                return true;
            },
            action: ({ model, rootComponent }) => {
                ...
                if (!model.doc.root)
                    return;
                const widgetEle = rootComponent.widgetComponents['affine-linked-doc-widget'];
                if (!widgetEle)
                    return;
                // We have checked the existence of showLinkedDoc method in the showWhen
                const linkedDocWidget = widgetEle;
        ...
        { groupName: 'Content & Media' },
        {
            ...
            action: async ({ rootComponent, model }) => {
                ...
                addSiblingImageBlock(rootComponent.host, imageFiles, maxFileSize, model);
        ...
        ({ model, rootComponent }) => {
            ...
            const frameItems = frameModels.map(frameModel => ({
                ...
                action: () => {
                    ...
                    doc.addSiblingBlocks(model, [surfaceRefProps], insertIdx === 0 ? 'before' : 'after');
            ...
            const groupElements = Array.from(surfaceModel.elements.getValue()?.values() ?? []).filter(element => element.get('type') === CanvasElementType.GROUP);
        ...
        { groupName: 'Database' },
        {
            ...
            action: ({ rootComponent, model }) => {
                ...
                service.initDatabaseBlock(rootComponent.doc, model, id, viewPresets.tableViewConfig, false);
                tryRemoveEmptyLine(model);
            },
        },
        {
        ...
        },
        {
            ...
            action: ({ model, rootComponent }) => {
                ...
                const service = rootComponent.std.getService('affine:database');
                service.initDatabaseBlock(rootComponent.doc, model, id, viewPresets.kanbanViewConfig, false);
                tryRemoveEmptyLine(model);
            },
        },
        // ---------------------------------------------------------
