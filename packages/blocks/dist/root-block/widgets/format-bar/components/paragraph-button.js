...
const ParagraphPanel = ({ formatBar, host, ref: containerRef, }) => {
    ...
    const renderedConfig = repeat(config, item => html `
      <editor-menu-action
        data-testid="${item.id}"
...
export const ParagraphButton = (formatBar) => {
    ...
    const paragraphIcon = selectedBlocks.length < 1
        ? textConversionConfigs[0].icon
        : (textConversionConfigs.find(({ flavour, type }) => selectedBlocks[0].flavour === flavour &&
            selectedBlocks[0].model.type === type)?.icon ?? textConversionConfigs[0].icon);
