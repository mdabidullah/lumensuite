...
export class TableSelectionController {
    ...
    handleSelectionChange() {
        this.host.disposables.add(this.host.selection$.subscribe(tableSelection => {
    ...
    insertTo(groupKey, rowId, before) {
        ...
        requestAnimationFrame(() => {
            const index = this.host.view.columnManagerList$.value.findIndex(v => v.type$.value === 'title');
    ...
    navigateRowSelection(direction, append = false) {
        if (!TableRowSelection.is(this.selection))
            return;
  ...
  `; }
    ...
    `;
        ...
        updateAreaSelectionStyle(groupKey, rowSelection, columnSelection) {
            ...
            const rect = this.controller.getRect(groupKey, rowSelection?.start ?? 0, rowSelection?.end ?? this.controller.view.rows$.value.length - 1, columnSelection?.start ?? 0, columnSelection?.end ??
