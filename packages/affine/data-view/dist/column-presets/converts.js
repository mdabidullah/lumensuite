...
export const presetColumnConverts = [
    createColumnConvert(multiSelectColumnModelConfig, selectColumnModelConfig, (column, cells) => ({
        column,
        cells: cells.map(v => v?.[0]),
    })),
    createColumnConvert(numberColumnModelConfig, progressColumnModelConfig, (_column, cells) => ({
        column: {},
        cells: cells.map(v => clamp(v ?? 0, 0, 100)),
    })),
    createColumnConvert(progressColumnModelConfig, numberColumnModelConfig, (_column, cells) => ({
        column: {
            decimal: 0,
            format: 'number',
        },
    ...
    createColumnConvert(selectColumnModelConfig, multiSelectColumnModelConfig, (column, cells) => ({
        column,
