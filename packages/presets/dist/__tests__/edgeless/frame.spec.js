...
describe('frame', () => {
    ...
    test('frame should have title', async () => {
        ...
        await wait();
        const titleDom = document.querySelector(`affine-frame[data-block-id="${frame}"] .affine-frame-title`);
        ...
        await wait();
        const nestedTitle = document.querySelector(`affine-frame[data-block-id="${nestedFrame}"] .affine-frame-title`);
    ...
    test('frame should always be placed under the bottom of other blocks', async () => {
        ...
        const frameTitle = document.querySelector(`[data-block-id="${frameId}"] .affine-frame-title`);
        const titleRect = frameTitle.getBoundingClientRect();
        const frameBody = document.querySelector(`[data-block-id="${frameId}"] .affine-frame-container`);
        const bodyRect = frameBody.getBoundingClientRect();
        const pointDom1 = document.elementFromPoint(titleRect.x + titleRect.width / 2, titleRect.y + titleRect.height / 2);
        const pointDom2 = document.elementFromPoint(bodyRect.x + bodyRect.width / 2, bodyRect.y + bodyRect.height / 2);
        expect(frameTitle.contains(pointDom1), 'Frame title should be on top').toBeTruthy();
        expect(frameBody.contains(pointDom2), 'Frame body should be on bottom').toBeFalsy();
    });
