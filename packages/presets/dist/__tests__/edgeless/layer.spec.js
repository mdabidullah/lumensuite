...
test('blocks should rerender when their z-index changed', async () => {
    ...
    service.addElement('shape', {
        shapeType: 'rect',
        index: CommonUtils.generateKeyBetween(service.getElementById(blocks[1]).index, service.getElementById(blocks[2]).index),
    });
    await wait();
    assertBlocksContent();
});
...
describe('compare function', () => {
    ...
    test('compare two different elements', () => {
        ...
        const note1 = service.getElementById(note1Id);
        const note2Id = addNote(doc, {
            index: service.layer.generateIndex('shape'),
        });
        const note2 = service.getElementById(note2Id);
